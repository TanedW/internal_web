...
const FlexBox = ({ node, parentLayout }) => {
  ...
  // 🟢 FIX: Default Width Logic to 'auto' to prevent expansion
  ...
  // Default to stretch to fill width in vertical stacks naturally
...
const FlexImage = ({ node, parentLayout }) => {
  const style = {
    ...
    // 🟢 FIX: Use 'image-width' type so 'sm' is 32px, not 60%
    width: node.size === 'full' ? '100%' : getSize(node.size, 'image-width'),
...
const FlexSeparator = ({ node, parentLayout }) => {
    ...
    const marginVal = node.margin ? getSpacing(node.margin) : '0px'; 
  ...
  switch (node.type) {
    ...
    case 'bubble':
        ...
        const bubbleStyle = {
            ...
            maxWidth: '100%',
            // 🟢 FIX: Ensure bubble has a proper base width
            width: bubbleWidth 
        };
        ...
        return (
            <div style={bubbleStyle} className="fl-bubble">
                ...
                {/* 🟢 Body with Auto-Padding Logic */}
                {node.body && (
                    <div style={{
                        flex: 1, 
                        display: 'flex', 
                        flexDirection: 'column', 
                        backgroundColor: node.body.backgroundColor,
                        // If no padding defined, add a default 8px safe area
                        padding: node.body.paddingAll ? '0px' : '8px' 
                    }}>
