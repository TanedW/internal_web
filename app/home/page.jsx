...
import { 
  ...
  LayoutDashboard, ShieldCheck, LogOut // เพิ่ม LogOut
} from "lucide-react";
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    // ยืนยันว่า Client พร้อมแสดงผลแล้ว
    setIsLoaded(true);
  }, []);
  ...
  const handleLogout = () => {
    ...
    if (confirmLogout) {
      sessionStorage.clear(); // ล้างข้อมูลทั้งหมดใน Session
      window.location.href = "/"; // กลับไปยังหน้า Login/Landing
    }
  };

  const allMenus = useMemo(() => [
    { title: "จัดการ Email", slug: "email", description: "ดูแลสิทธิ์สมาชิกและระบบรับส่งอีเมล", icon: <Mail size={24} />, href: "/manage", roles: ["admin", "editor", "editor_manage_user"] },
    { title: "จัดการ Case", slug: "case", description: "ติดตามความคืบหน้าและบริหารจัดการเคส", icon: <Briefcase size={24} />, href: "/manage-case", roles: ["admin", "editor", "editor_manage_case"] },
    { title: "จัดการ Menu", slug: "menu", description: "ออกแบบ Rich Menu สำหรับ LINE OA", icon: <LayoutGrid size={24} />, href: "/manage-richmenu", roles: ["admin", "editor", "editor_manage_menu"] },
    { title: "จัดการ ORG", slug: "org", description: "โครงสร้างหน่วยงานและข้อมูลองค์กร", icon: <Users size={24} />, href: "/manage-org", roles: ["admin", "editor", "editor_manage_org", "editor_manage_org_info"] },
    { title: "จัดการ Flex Message", slug: "flex", description: "สร้างชุดข้อความ Flex แบบมืออาชีพ", icon: <MessageSquareCode size={24} />, href: "/manage-flex-message", roles: ["admin", "editor", "editor_manage_flex"] },
    { title: "ตรวจสอบข้อมูลซ้ำ", slug: "search", description: "ค้นหาและจัดการความซ้ำซ้อนของหน่วยงาน", icon: <Search size={24} />, href: "/search-org", roles: ["admin", "editor", "editor_search_duplicate_org"] },
    { title: "คลังไฟล์ FAQ", slug: "file", description: "จัดการฐานข้อมูลความรู้และเอกสาร", icon: <FolderSearch size={24} />, href: "/manage-file-search", roles: ["admin", "editor", "editor_file_search"] },
  ], []);

  const accessibleMenus = isLoaded 
    ? allMenus.filter(menu => currentRoles.some(r => menu.roles.includes(r)))
    : [];
        ...
        <div className={`transition-opacity duration-500 ${isLoaded ? 'opacity-100' : 'opacity-0'}`}>
          ...
          <header className="mb-16">
            ...
            <p className="text-slate-500 font-medium text-lg lg:text-xl max-w-2xl leading-relaxed">
              เข้าถึงเครื่องมือจัดการข้อมูลทั้งหมดของคุณได้จากที่นี่ เลือกโมดูลที่คุณต้องการเพื่อเริ่มดำเนินการ
            </p>
          </header>
