...
// ── AvatarStack: แสดง avatar ตาม user count ──
const AvatarStack = ({ count = 0, accent = '#3B82F6' }) => {
  if (count === 0)
    return <span className="text-[11px] text-gray-400 italic">ยังไม่มี user</span>;
  return (
    <div className="flex items-center gap-1.5">
      <div className="flex">
        {[...Array(Math.min(count, 3))].map((_, i) => (
          <div
            ...
            style={{ background: accent, color: '#fff', marginLeft: i > 0 ? -8 : 0 }}
          >
            U
          </div>
        ))}
      ...
      {count > 3 && (
        <span className="text-[11px] font-bold text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full border border-gray-200 ml-1">
          +{count - 3}
        </span>
      )}
    </div>
...
export default function RichMenuDashboard() {
  ...
  const [segmentMenus, setSegmentMenus] = useState({}); // { [segId]: [{id,rich_menu_id,rich_menu_name,...}] }
  const [addMenuModal, setAddMenuModal] = useState(null);   // segment | null
  const [addMenuSelectedId, setAddMenuSelectedId] = useState('');
  ...
  async function fetchSegmentUsers(segmentId) {
    ...
    try {
      ...
      // เก็บ active menus ของ segment นี้ด้วย
      const activeMenus = (data.menu_history || []).filter((m) => m.is_active);
      setSegmentMenus((prev) => ({ ...prev, [segmentId]: activeMenus }));
    } catch {
      setSegmentUsers([]);
    } finally {
  ...
  async function handleSaveSegment() {
    ...
    try {
      ...
      const body = isEdit
        ? {
            segmentId: segmentFormModal.data.id,
            ...segmentForm,
            adminEmail: user?.email,
          }
        : {
      ...
      const res = await fetch(
        `${API}?action=${isEdit ? "update_segment" : "create_segment"}`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
    ...
    <div className="min-h-screen bg-[#F4F6F8] font-sans">
          ...
          <div className="php-container">
            ...
            <div className="php-qa-grid animate-fade-in-up">
              ...
              <button
                onClick={() =>
                  document
                    .getElementById("segment-section")
                        ...
                        return filtered.map((log, i) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              {log.action?.startsWith("MENU_SWITCH") &&
            ...
            <section id="segment-section" className="php-card">
              ...
              <div className="flex items-start gap-3 mb-8">
                ...
                <div>
                  <h2 className="text-[18px] font-black text-slate-900 m-0 tracking-tight leading-tight">
                    จัดการลำดับการแสดงผลเมนู
                  </h2>
                  <p className="text-xs text-slate-500 mt-0.5">
                    กำหนดว่าใครจะเห็นเมนูไหน โดยแบ่งความสำคัญเป็น 3 ระดับ (Level 1 ถึง 3)
                  </p>
                </div>
              </div>
              ...
              {(() => {
                ...
                const tagSegs = segments.filter((s) => !s.is_default);
                const filteredTagSegs = tagSegs.filter((s) =>
                  s.name.toLowerCase().includes(segmentSearch.toLowerCase())
                );

                // helper: get primary menu image src for a segment
                const getPrimaryImgSrc = (seg) => {
                ...
                // helper: get menu count for a segment
                const getMenuCount = (seg) => segmentMenus[seg.id] !== undefined
                  ? segmentMenus[seg.id].length
                  : (seg.active_menu_count ?? (seg.active_rich_menu_name ? 1 : 0));

                // find active menu name for default seg
                const defaultMenuEntry = defaultSeg
                  ? ((segmentMenus[defaultSeg.id] || [])[0] || (defaultSeg.active_rich_menu_id ? { rich_menu_id: defaultSeg.active_rich_menu_id, rich_menu_name: defaultSeg.active_rich_menu_name } : null))
                  : null;
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="flex items-stretch gap-4">
                      ...
                      <div className="flex flex-col items-center">
                        <div className="bg-slate-800 text-white text-[10px] font-black px-2.5 py-1 rounded-lg tracking-widest uppercase whitespace-nowrap">
                          LEVEL 1
                        </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-3">
                          <span className="font-black text-slate-800 text-sm">เมนูเริ่มต้น</span>
                          <span className="text-slate-400 text-xs">— แสดงเมื่อผู้ใช้ยังไม่ระบุกัด และไม่มีสิทธิ์พิเศษ</span>
                        </div>

                        {defaultSeg ? (
                          <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm flex flex-col sm:flex-row">
                            {/* Preview image */}
                            <div className="sm:w-48 aspect-[2500/843] sm:aspect-auto bg-slate-50 relative border-b sm:border-b-0 sm:border-r border-slate-100 shrink-0">
                              {getPrimaryImgSrc(defaultSeg) ? (
                                <img
                                  src={getPrimaryImgSrc(defaultSeg)}
                                  alt={defaultSeg.name}
                                  className="w-full h-full object-cover"
                                  onError={(e) => { e.target.style.display = 'none'; }}
                                />
                              ) : (
                                <div className="w-full h-full min-h-[80px] flex flex-col items-center justify-center gap-1 text-slate-300">
                                  <Globe size={24} />
                                  <span className="text-[10px] font-bold">Welcome Default Menu</span>
                                </div>
                              )}
                            </div>
                            ...
                            <div className="p-5 flex-1 flex flex-col">
                              <div className="flex items-start gap-3 mb-3">
                                ...
                                <div className="min-w-0">
                                  <div className="font-black text-slate-900 text-sm">{defaultSeg.name}</div>
                                  <div className="text-[10px] text-slate-400 font-mono mt-0.5">ID: {defaultSeg.id}</div>
                                </div>
                              </div>
                              <div className="text-xs text-slate-500 bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 mb-4 leading-relaxed">
                                เมนูนี้จะถูกแสดงเป็นค่าเริ่มต้นสำหรับผู้ใช้ทุกคนที่ยังไม่มีการจัดกลุ่ม (Tag) หรือไม่มีสิทธิ์พิเศษ
                              </div>
                              <div className="flex gap-2 mt-auto">
                                <button
                                  onClick={() => { setUsersModal(defaultSeg); fetchSegmentUsers(defaultSeg.id); }}
                                  className="flex-1 py-2 px-3 bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 transition-colors shadow-sm"
                                >
                                ...
                                <button
                                  onClick={() => {
                                    if (defaultMenuEntry) {
                                      setAssignSelectedMenuId(defaultMenuEntry.rich_menu_id);
                                      setAssignMenuModal(defaultSeg);
                                    } else {
                                      setAddMenuSelectedId('');
                                      setAddMenuModal(defaultSeg);
                                    }
                                  }}
                                  className="flex-1 py-2 px-3 bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 transition-colors shadow-sm"
                                >
                          ...
                          <div className="bg-white border-2 border-dashed border-slate-200 rounded-2xl p-8 text-center text-slate-400">
                            ...
                            <p className="font-bold text-slate-500 text-sm">ยังไม่มีเมนูเริ่มต้น</p>
                            <p className="text-xs mt-1">สร้างพื้นที่และติ๊ก "ตั้งเป็นกลุ่มเริ่มต้น" เพื่อกำหนด Default Menu</p>
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex items-stretch gap-4">
                      {/* Level Badge */}
                      <div className="flex flex-col items-center">
                        <div className="bg-blue-600 text-white text-[10px] font-black px-2.5 py-1 rounded-lg tracking-widest uppercase whitespace-nowrap">
                          LEVEL 2
                        </div>
                        <div className="w-px flex-1 bg-slate-200 mt-2" />
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
                          <div className="flex items-center gap-2">
                            <span className="font-black text-slate-800 text-sm">แยกตาม Tag</span>
                            <span className="text-slate-400 text-xs">— แสดงตาม Tag อัตโนมัติได้จากการแจ้งพิกัด</span>
                          </div>
                          <div className="flex items-center gap-2 shrink-0">
                            <div className="relative">
                              ...
                              <input
                                ...
                                className="pl-7 pr-3 py-1.5 bg-white border border-slate-200 rounded-lg text-xs outline-none focus:border-blue-400 shadow-sm transition-all w-44"
                              />
                            </div>
                            <button
                              onClick={() => {
                              ...
                              className="text-xs font-bold text-blue-600 hover:text-blue-800 flex items-center gap-1 transition-colors bg-blue-50 px-3 py-1.5 rounded-lg border border-blue-100 shadow-sm"
                            >
                              <Plus size={13} /> เพิ่ม Tag
                            </button>
                          </div>
                        </div>
                        ...
                        {tagSegs.length === 0 ? (
                          <div className="py-10 text-center bg-white rounded-2xl border-2 border-dashed border-slate-200 text-slate-400">
                            ...
                            <p className="font-bold text-slate-500 text-sm">ยังไม่มีการตั้งค่าพื้นที่/Tag</p>
                            <p className="text-xs mt-1">กดปุ่ม "+ เพิ่ม Tag" เพื่อเริ่มต้น</p>
                          </div>
                        ) : filteredTagSegs.length === 0 ? (
                          <div className="py-8 text-center text-slate-400">
                            <Search size={24} className="mx-auto mb-2 opacity-30" />
                            <p className="font-bold text-sm">ไม่พบพื้นที่ที่ค้นหา</p>
                          </div>
                        ) : (
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {filteredTagSegs.map((seg, segIdx) => {
                              const COLORS = [
                                { bg: '#F0FFF4', text: '#166534', border: '#86EFAC', imgBg: '#D1FAE5', userCount: '#059669' },
                                { bg: '#FEF2F2', text: '#991B1B', border: '#FCA5A5', imgBg: '#FEE2E2', userCount: '#DC2626' },
                                { bg: '#EFF6FF', text: '#1D4ED8', border: '#93C5FD', imgBg: '#DBEAFE', userCount: '#2563EB' },
                                { bg: '#FFF7ED', text: '#92400E', border: '#FCD34D', imgBg: '#FEF3C7', userCount: '#D97706' },
                                { bg: '#F5F3FF', text: '#5B21B6', border: '#C4B5FD', imgBg: '#EDE9FE', userCount: '#7C3AED' },
                              ];
                              const col = COLORS[segIdx % COLORS.length];
                              const imgSrc = getPrimaryImgSrc(seg);
                              const menuCount = getMenuCount(seg);
                              const segMenuList = segmentMenus[seg.id];
                              const displayMenus = segMenuList !== undefined
                                ? segMenuList
                                : (seg.active_rich_menu_id
                                    ? [{ id: null, rich_menu_id: seg.active_rich_menu_id, rich_menu_name: seg.active_rich_menu_name }]
                                    : []);
                              const primaryEntry = displayMenus[0] || null;
                              ...
                              return (
                                <div
                                  key={seg.id}
                                  className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm flex flex-col transition-all hover:shadow-md"
                                >
                                  {/* Banner: ชื่อจังหวัด / tag */}
                                  <div
                                    className="relative w-full flex items-center justify-center py-10 px-4"
                                    style={{ background: col.imgBg, minHeight: 120 }}
                                  >
                                    {imgSrc ? (
                                      <img
                                        src={imgSrc}
                                    ...
                                    {/* Kebab menu (edit/delete) top-right */}
                                    <div className="absolute top-2 right-2 group/kebab z-20">
                                      <button className="w-7 h-7 bg-white/80 hover:bg-white border border-slate-200 rounded-lg flex items-center justify-center text-slate-500 shadow-sm transition-colors">
                                        <ChevronDown size={12} />
                                      </button>
                                      <div className="absolute right-0 top-full mt-1 w-36 bg-white border border-slate-200 rounded-xl shadow-xl z-30 overflow-hidden hidden group-hover/kebab:block">
                                        <button
                                  ...
                                  <div className="px-4 pt-3 pb-1 flex items-center justify-between">
                                    <div className="flex items-center gap-1.5 text-[11px] text-slate-500">
                                      ...
                                      <span>จังหวัด: {seg.description || seg.name}</span>
                                    </div>
                                    <div className="text-right">
                                      <div className="font-black text-slate-800 text-base leading-none" style={{ color: col.userCount }}>
                                        {(seg.user_count ?? 0).toLocaleString()}
                                      </div>
                                      <div className="text-[9px] text-slate-400 uppercase font-bold tracking-widest">USERS</div>
                                    </div>
                                  </div>
                                  ...
                                  {displayMenus.length > 1 && (
                                    <div className="px-4 pb-2 flex flex-wrap gap-1">
                                      {displayMenus.map((entry) => (
                                        <span
                                          key={entry.id || entry.rich_menu_id}
                                          className="text-[10px] font-bold px-2 py-0.5 rounded-full border"
                                          style={{ background: col.imgBg, color: col.text, borderColor: col.border }}
                                        >
                                  ...
                                  {/* Action buttons */}
                                  <div className="px-4 pb-4 pt-2 border-t border-slate-100 mt-2 flex gap-2">
                                    <button
                                      onClick={() => { setUsersModal(seg); fetchSegmentUsers(seg.id); }}
                                      className="flex-1 py-2 bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 transition-colors shadow-sm"
                                    >
                                      <Users size={12} className="text-slate-400" />
                                      ดูรายชื่อผู้ใช้งาน ({seg.user_count ?? 0})
                                    </button>
                                    <button
                                      onClick={() => {
                                        if (primaryEntry) {
                    ...
                    <div className="flex items-stretch gap-4">
                      {/* Level Badge */}
                      <div className="flex flex-col items-center">
                        <div className="bg-rose-500 text-white text-[10px] font-black px-2.5 py-1 rounded-lg tracking-widest uppercase whitespace-nowrap">
                          LEVEL 3
                        </div>
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center gap-2">
                            <span className="font-black text-slate-800 text-sm">บังคับสิทธิ์รายบุคคล</span>
                            <span className="text-slate-400 text-xs">— ระดับสูงสุด บังคับให้เห็นเมนูที่กำหนด ข้ามกฎทั้งหมดทันที</span>
                          </div>
                          <button
                            onClick={() => {
                              // หาก segment ที่มี user overrides อยู่ เปิด modal users ของ segment แรก
                              const overrideSeg = segments.find(s => (s.override_user_count ?? 0) > 0) || segments[0];
                              if (overrideSeg) { setUsersModal(overrideSeg); fetchSegmentUsers(overrideSeg.id); }
                            }}
                            className="text-xs font-bold text-rose-600 hover:text-rose-800 flex items-center gap-1 bg-rose-50 px-3 py-1.5 rounded-lg border border-rose-100 shadow-sm transition-colors shrink-0"
                          >
                        ...
                        {/* Overview card */}
                        {(() => {
                          const totalOverride = segments.reduce((sum, s) => sum + (s.override_user_count ?? 0), 0);
                          // จำนวน user ที่ถูก override (รวมทุก segment ที่ไม่ใช่ default และมี override)
                          // fallback: ถ้าไม่มี override_user_count ให้ดูจาก segments ที่มี user_count โดยตรง
                          const overrideCount = totalOverride > 0
                            ? totalOverride
                            : segments.filter(s => !s.is_default).reduce((sum, s) => sum + (s.override_user_count ?? 0), 0);
                          ...
                          return (
                            <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex items-start gap-4">
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="font-black text-slate-800 text-sm mb-0.5">รายชื่อที่ถูกบังคับสิทธิ์ทั้งหมด</div>
                                <p className="text-xs text-slate-500 leading-relaxed">
                                  ปัจจุบันมีผู้ใช้{' '}
                                  <span className="font-black text-rose-600">
                                    {overrideCount > 0 ? overrideCount : '—'} {overrideCount > 0 ? 'คน' : ''}
                                  </span>
                              ...
                              <div className="flex gap-1.5 shrink-0">
                                <button
                                  className="w-8 h-8 rounded-lg bg-slate-100 hover:bg-slate-200 flex items-center justify-center text-slate-600 font-black text-xs transition-colors"
                                  title="ส"
                                >ส</button>
                                <button
                                  className="w-8 h-8 rounded-lg bg-slate-100 hover:bg-slate-200 flex items-center justify-center text-slate-600 font-black text-xs transition-colors"
      ...
      {addMenuModal && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm" onClick={() => setAddMenuModal(null)} />
          <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-lg flex flex-col max-h-[88vh] overflow-hidden z-10">
            <div className="p-5 border-b border-gray-100 flex justify-between items-start bg-slate-50/60">
              <div>
                <div className="text-[10px] font-bold text-green-600 bg-green-100 px-2 py-0.5 rounded-full inline-flex items-center gap-1 mb-2">
                  <MapPin size={9} /> {addMenuModal.name}
                </div>
                <h2 className="text-lg font-black text-gray-900">เพิ่มเมนูสำหรับพื้นที่นี้</h2>
                <p className="text-xs text-gray-400 mt-0.5">เลือกเมนูที่ต้องการเพิ่มเข้ากลุ่มนี้</p>
              </div>
              <button onClick={() => setAddMenuModal(null)} className="p-2 bg-white border border-gray-200 hover:bg-gray-100 rounded-full transition-colors">
                <X size={16} />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 bg-[#F8FAFC] space-y-2">
              <div className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3">
                เลือกเมนู ({menus.length} รายการ)
              </div>
              {menus.length === 0 && (
                <p className="text-center text-sm text-slate-400 py-8">ไม่มีเมนูในระบบ กรุณาอัปโหลดเมนูก่อน</p>
              )}
              {menus.map((menu) => {
                const isSelected = menu.richMenuId === addMenuSelectedId;
                const alreadyAdded = (segmentMenus[addMenuModal.id] || []).some((m) => m.rich_menu_id === menu.richMenuId);
                return (
                  <button
                    key={menu.richMenuId}
                    onClick={() => !alreadyAdded && setAddMenuSelectedId(menu.richMenuId)}
                    disabled={alreadyAdded}
                    className={`w-full flex items-center gap-3 p-3.5 rounded-xl border-2 text-left transition-all ${
                      alreadyAdded ? 'border-gray-100 bg-gray-50 opacity-50 cursor-not-allowed'
                      : isSelected ? 'border-green-400 bg-green-50/60 shadow-sm'
                      : 'border-transparent hover:border-gray-200 bg-white shadow-sm'
                    }`}
                  >
                    <div className="w-20 h-7 rounded-lg overflow-hidden bg-slate-100 shrink-0 border border-slate-200">
                      {menu.image_url && <img src={menu.image_url} alt="" className="w-full h-full object-cover" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className={`text-sm font-bold truncate ${isSelected ? 'text-gray-900' : 'text-gray-700'}`}>{menu.name}</span>
                        {alreadyAdded && <span className="text-[9px] px-1.5 py-0.5 bg-gray-200 text-gray-500 font-black rounded-full shrink-0">มีแล้ว</span>}
                      </div>
                      <p className="text-[10px] text-gray-400 font-mono truncate mt-0.5">{menu.richMenuId}</p>
                    </div>
            ...
            <div className="p-5 border-t border-gray-100 bg-white flex gap-3">
              <button onClick={() => setAddMenuModal(null)} className="flex-1 py-2.5 rounded-xl border border-gray-200 text-slate-600 font-bold text-sm hover:bg-gray-50 transition-all">
                ยกเลิก
              </button>
              <button
                onClick={handleAddSegmentMenu}
                disabled={!addMenuSelectedId || addingMenu}
                className="flex-1 py-2.5 rounded-xl bg-green-600 hover:bg-green-700 disabled:bg-slate-200 text-white font-bold text-sm flex items-center justify-center gap-2 transition-all"
              >
                {addingMenu ? <Loader2 size={15} className="animate-spin" /> : <Plus size={15} />}
                {addingMenu ? 'กำลังเพิ่ม...' : 'เพิ่มเมนูนี้'}
              </button>
            </div>
          </div>
        </div>
      ...
      {segmentFormModal && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm" onClick={() => setSegmentFormModal(null)} />
          <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden z-10">
            <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-slate-50/60">
              <h2 className="text-lg font-black text-gray-900">
                {segmentFormModal.mode === 'edit' ? 'แก้ไขพื้นที่' : 'เพิ่มพื้นที่ใหม่'}
              </h2>
              <button onClick={() => setSegmentFormModal(null)} className="p-2 bg-white border border-gray-200 hover:bg-gray-100 rounded-full transition-colors">
                <X size={16} />
              </button>
            </div>
            <div className="p-6 space-y-4">
              <div>
                <label className="text-[11px] font-bold text-slate-500 mb-1.5 block">ชื่อพื้นที่ / จังหวัด</label>
                <input
                  value={segmentForm.name}
                  onChange={(e) => setSegmentForm({ ...segmentForm, name: e.target.value })}
                  placeholder="เช่น กรุงเทพ, เชียงใหม่, ยะลา"
                  className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:bg-white focus:border-blue-400 transition-colors"
                />
              </div>
              <div>
                <label className="text-[11px] font-bold text-slate-500 mb-1.5 block">คำอธิบาย (ไม่บังคับ)</label>
                <input
              ...
              <label className="flex items-center gap-3 cursor-pointer p-3 rounded-xl border border-gray-100 hover:bg-gray-50 transition-colors">
                ...
                />
                <div>
                  <div className="text-sm font-bold text-slate-700">ตั้งเป็นกลุ่มเริ่มต้น (Default)</div>
                  <div className="text-xs text-slate-400 mt-0.5">ผู้ใช้ที่ไม่ได้อยู่ในกลุ่มอื่น จะเห็น Rich Menu ของกลุ่มนี้</div>
                </div>
              </label>
            </div>
            <div className="p-5 border-t border-gray-100 flex gap-3">
              <button onClick={() => setSegmentFormModal(null)} className="flex-1 py-2.5 rounded-xl border border-gray-200 text-slate-600 font-bold text-sm hover:bg-gray-50 transition-all">
                ยกเลิก
              </button>
              <button
                onClick={handleSaveSegment}
                disabled={!segmentForm.name.trim() || savingSegment}
                className="flex-1 py-2.5 rounded-xl bg-slate-900 hover:bg-slate-700 disabled:bg-slate-200 text-white font-bold text-sm flex items-center justify-center gap-2 transition-all"
              >
      ...
      {assignMenuModal && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm" onClick={() => setAssignMenuModal(null)} />
          <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-lg flex flex-col max-h-[88vh] overflow-hidden z-10">
            <div className="p-5 border-b border-gray-100 flex justify-between items-start bg-slate-50/60">
              <div>
                <div className="text-[10px] font-bold text-blue-600 bg-blue-100 px-2 py-0.5 rounded-full inline-flex items-center gap-1 mb-2">
                  <MapPin size={9} /> {assignMenuModal.name}
                </div>
                <h2 className="text-lg font-black text-gray-900">Assign เมนู</h2>
              </div>
              <button onClick={() => setAssignMenuModal(null)} className="p-2 bg-white border border-gray-200 hover:bg-gray-100 rounded-full transition-colors">
                <X size={16} />
              </button>
            </div>
            <div className="flex-1 overflow-y-auto p-4 bg-[#F8FAFC] space-y-2">
              ...
              {menus.map((menu) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 p-3.5 rounded-xl border-2 text-left transition-all ${isSelected ? 'border-blue-400 bg-blue-50/60 shadow-sm' : 'border-transparent hover:border-gray-200 bg-white shadow-sm'}`}
                  >
                    <div className="w-20 h-7 rounded-lg overflow-hidden bg-slate-100 shrink-0 border border-slate-200">
                      {menu.image_url && <img src={menu.image_url} alt="" className="w-full h-full object-cover" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className={`text-sm font-bold truncate ${isSelected ? 'text-gray-900' : 'text-gray-700'}`}>{menu.name}</span>
                        {isCurrent && <span className="text-[9px] px-1.5 py-0.5 bg-green-100 text-green-700 font-black rounded-full shrink-0">ใช้อยู่</span>}
                      </div>
                      <p className="text-[10px] text-gray-400 font-mono truncate mt-0.5">{menu.richMenuId}</p>
                    </div>
            ...
            <div className="p-5 border-t border-gray-100 bg-white flex gap-3">
              <button onClick={() => setAssignMenuModal(null)} className="flex-1 py-2.5 rounded-xl border border-gray-200 text-slate-600 font-bold text-sm hover:bg-gray-50 transition-all">
                ยกเลิก
              </button>
              <button
      ...
      {usersModal && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm" onClick={() => setUsersModal(null)} />
          <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-lg flex flex-col max-h-[88vh] overflow-hidden z-10">
            <div className="p-5 border-b border-gray-100 flex justify-between items-start bg-slate-50/60">
              <div>
                <div className="text-[10px] font-bold text-blue-600 bg-blue-100 px-2 py-0.5 rounded-full inline-flex items-center gap-1 mb-2">
                ...
                <h2 className="text-lg font-black text-gray-900">จัดการ Users</h2>
              </div>
              <button onClick={() => setUsersModal(null)} className="p-2 bg-white border border-gray-200 hover:bg-gray-100 rounded-full transition-colors">
                <X size={16} />
              </button>
            </div>
            <div className="p-4 border-b border-gray-100 bg-white">
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">เพิ่ม User</p>
              <div className="flex gap-2">
                <input
                  value={newUserId}
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-full bg-gray-100 flex items-center justify-center font-bold text-sm text-gray-500">
                        {(u.display_name || u.line_user_id || '?').charAt(0).toUpperCase()}
                      </div>
                      <div>
                        <div className="text-sm font-bold text-gray-700">
                          {u.display_name || <span className="text-gray-400 italic text-xs">ไม่ทราบชื่อ</span>}
                        </div>
                        <div className="text-[10px] text-gray-400 font-mono">
                          {u.line_user_id}
                          <span className="bg-gray-200 text-gray-600 px-1.5 rounded ml-1.5">{u.source}</span>
                        </div>
                      </div>
                    </div>
                    <button
            ...
            <div className="p-4 border-t border-gray-100 bg-white">
              <button onClick={() => setUsersModal(null)} className="w-full py-2.5 bg-slate-900 text-white rounded-xl text-sm font-bold">
                ปิด
              </button>
            </div>
          </div>
