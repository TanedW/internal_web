...
const refreshAdminProfileInBackground = async () => {
  ...
  const SidebarRoleDisplay = () => (
    <div className="flex flex-col items-center mt-2 px-2 w-full">
      {currentRoles.length > 0 ? (
        ...
        isDesktopSidebarOpen && <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Guest</span>
      )}
    </div>
  );
  ...
  const SidebarHeader = () => (
    <div className="flex flex-col items-center text-center mb-8 mt-2 transition-all">
      <div className={`rounded-full overflow-hidden border-4 border-white shadow-lg bg-slate-100 flex items-center justify-center transition-all duration-300 ${isDesktopSidebarOpen ? 'w-24 h-24' : 'w-10 h-10'}`}>
        <img
  ...
  const SidebarSkeleton = () => (
    <div className="flex flex-col items-center w-full animate-pulse">
      <div className={`${isDesktopSidebarOpen ? 'w-24 h-24' : 'w-10 h-10'} rounded-full bg-slate-200 mb-8`}></div>
      {isDesktopSidebarOpen && (
        <>
              ...
              <div className="flex flex-col h-full animate-in fade-in duration-500">
                ...
                <nav className="flex flex-col gap-1.5 flex-1 mt-6 overflow-y-auto no-scrollbar">
                  ...
                  {hasAccess(["admin", "editor", "editor_file_search"]) && (
                    <Link href="/manage-file-search" className={getMenuClass("/manage-file-search")} onClick={() => setIsMobileMenuOpen(false)}>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col h-full animate-in fade-in duration-500 overflow-hidden">
            ...
            <div className={`mt-auto pt-4 border-t border-slate-100 ${!isDesktopSidebarOpen && 'flex justify-center'}`}>
              <button onClick={handleLogout} className={`group flex items-center ${isDesktopSidebarOpen ? 'gap-2.5 px-4' : 'justify-center'} py-3 rounded-xl hover:bg-red-50 transition-all duration-200 w-full`}>
                <div className="p-1.5 bg-red-100/50 rounded-lg group-hover:bg-red-100 transition-colors">
                  <LogOut size={20} className="text-red-500" />
                </div>
                {isDesktopSidebarOpen && <span className="text-red-600 font-bold tracking-wide text-[15px]">Logout</span>}
              </button>
            </div>
          </div>
        )}
