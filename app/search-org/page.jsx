    ...
    <div className="flex min-h-screen bg-[#f8fafc]">
      ...
      <main className={`flex-1 transition-all duration-300 pt-20 px-4 md:px-6 lg:p-8 ${isDesktopSidebarOpen ? "lg:ml-72" : "lg:ml-20"}`}>
        <div className="container-custom">
          ...
          <div id="resultsArea">
            {error && <div className="text-red-500 text-center p-4 bg-red-50 rounded-xl mb-4">ขออภัย: {error}</div>}

            {pagination && (
              <div className="mb-4">
                <div className="flex justify-between items-center mb-3">
                    <span className="font-semibold text-slate-700">พบ {pagination.total_records} รายการที่คล้ายคลึง</span>
                </div>
            ...
            <ul className="grid gap-4 list-none p-0">
                    ...
                    <div className="org-info flex-1">
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        <span 
                          ...
                          dangerouslySetInnerHTML={{ __html: item.highlighted_name || item.name }}
                        />
                        {item.tag_type && (
                          <span className="badge badge-official">
                            <CheckCircle2 size={14} className="text-blue-500" /> บัญชีทางการ
                          </span>
                        )}
                        {isDeleted && (
                          <span className="badge badge-deleted">
                            🗑 ลบเมื่อ {new Date(item.deleted_at).toLocaleDateString("th-TH", { day: 'numeric', month: 'short', year: '2-digit' })}
                          </span>
                        )}
                      </div>
              ...
              {!isLoading && results.length === 0 && !error && (
                <div className="text-center p-10 text-slate-400 bg-white rounded-2xl border-2 border-dashed border-slate-200">
                  {searchTerm ? "ไม่พบข้อมูลที่ซ้ำกัน คุณสามารถใช้ชื่อนี้ได้" : "กรุณากรอกชื่อหน่วยงานเพื่อเริ่มการค้นหา"}
                </div>
              )}
            </ul>
          </div>
