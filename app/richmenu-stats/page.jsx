...
import {
  Users, LayoutTemplate, Bot, Activity, ShieldCheck,
  RefreshCw, Trophy, AlertCircle, WifiOff,
} from "lucide-react";
  ...
  return (
    <div className="flex flex-col rounded-[2rem] bg-white border border-slate-50 shadow-sm p-6">
      <div className="mb-4">
        ...
        <p className="text-[13px] text-slate-400 mt-0.5 m-0">แยกตาม Rich Menu ID</p>
      </div>
  ...
  return (
    <div className="flex flex-col rounded-[2rem] bg-white border border-slate-50 shadow-sm overflow-hidden">
      ...
      <div className="flex items-start justify-between border-b border-slate-50 px-6 py-5">
        <div>
          <h2 className="text-[15px] font-bold text-slate-800 m-0">กระดานผู้นำ (Leaderboard)</h2>
        ...
        <table className="w-full min-w-[560px] border-collapse text-[13px] text-left">
          ...
          <tbody>
            {data.map((row, i) => {
              ...
              const rowUpdated = row.lastUpdate ? new Date(row.lastUpdate) : lastUpdated;
              return (
