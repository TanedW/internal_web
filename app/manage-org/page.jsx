...
// ✅ ดึง Sidebar จาก path ที่ถูกต้องตามโครงสร้างของคุณ (s ตัวเล็ก)
...
export default function ManageOrgPage() {
  ...
  const fetchOrgData = async (targetId = "") => {
    ...
    try {
      ...
      // Fallback ข้อมูลจำลองหาก API พัง
      ...
      <div className={`container mx-auto px-4 lg:px-8 pt-20 lg:pt-8 max-w-7xl transition-all duration-300 pb-24 ${
        ...
        {!isDesktopSidebarOpen && (
             <div className="hidden lg:block fixed top-8 left-8 z-30">
                <button onClick={() => setIsDesktopSidebarOpen(true)} className="p-2 bg-white rounded-xl shadow-md border border-slate-200 hover:bg-slate-50 transition-all">
              ...
              <div className="flex flex-col md:flex-row gap-6 items-center md:items-start">
                ...
                <div className="flex-1 space-y-5 w-full">
                  <div className="space-y-1.5">
                    ...
                    <input type="text" value={orgName} onChange={(e) => setOrgName(e.target.value)} className="w-full h-11 bg-white border border-slate-200 rounded-xl px-4 text-sm font-bold text-slate-900 focus:border-black outline-none transition-all shadow-sm" placeholder="กรอกชื่อหน่วยงาน..." />
                  </div>
