...
<div className="w-full bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
  {/* Tab Navigation */}
  <div className="flex border-b border-slate-50 px-8 pt-6 bg-white">
    <button 
      onClick={() => setActiveTab("urgent")} 
      className={`pb-4 px-6 text-sm font-black transition-all relative flex items-center gap-2 ${activeTab === "urgent" || activeTab === "manage" ? "text-indigo-600" : "text-slate-400 hover:text-slate-600"}`}
    >
      ต้องจัดการด่วน (12)
    ...
    <button 
      ...
      className={`pb-4 px-6 text-sm font-black transition-all relative flex items-center gap-2 ${activeTab === "history_edit" ? "text-indigo-600" : "text-slate-400 hover:text-slate-600"}`}
    >
      ประวัติการแก้ไขเคส
  ...
  <div className="p-4 sm:p-8 space-y-5 bg-[#FBFCFD]">
    {/* --- Content: ต้องจัดการด่วน --- */}
    {(activeTab === "urgent" || activeTab === "manage") && (
      <>
        {[
          { 
            id: "TCK-2024-0891", 
            title: "แจ้งปัญหาน้ำรั่วซึม", 
            status: "ขาดรูปปก", 
            statusColor: "bg-orange-50 text-orange-600",
            coverUrl: "https://images.unsplash.com/photo-1585704032915-c3400ca199e7?q=80&w=200&auto=format&fit=crop"
          },
          { 
            id: "TCK-2024-0885", 
            title: "เปลี่ยนหลอดไฟ", 
            ...
            coverUrl: "https://images.unsplash.com/photo-1550985616-10810253b84d?q=80&w=200&auto=format&fit=crop"
          },
          { 
            id: "TCK-2024-0870", 
            title: "แอร์ไม่เย็น", 
          ...
          { 
            id: "TCK-2024-0862", 
            title: "ซ่อมปริ้นเตอร์", 
            status: "ครบถ้วน", 
            statusColor: "bg-emerald-50 text-emerald-600",
            coverUrl: "https://images.unsplash.com/photo-1612815154858-60aa4c59eaa6?q=80&w=200&auto=format&fit=crop"
          },
        ].map((item, idx) => (
          <div 
            key={idx} 
            className="flex flex-col sm:flex-row items-center justify-between p-5 rounded-[2rem] bg-white border border-slate-200 shadow-[0_2px_8px_-2px_rgba(0,0,0,0.05)] hover:shadow-md hover:border-indigo-100 transition-all duration-300 group animate-fade-in"
          >
            ...
            <div className="flex items-center gap-4 w-full sm:w-auto justify-between sm:justify-end mt-4 sm:mt-0 pt-4 sm:pt-0 border-t sm:border-t-0 border-slate-50">
              <span className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest ${item.statusColor} bg-opacity-50 backdrop-blur-sm shadow-sm`}>
                {item.status}
              </span>
              <button 
                onClick={() => handleSearch(null, item.id)} 
                className="px-8 py-3 bg-[#FFB800] text-white font-black text-sm rounded-2xl hover:bg-[#F2AF00] hover:shadow-lg hover:shadow-yellow-100 active:scale-95 transition-all duration-200 whitespace-nowrap"
                >
                จัดการรูป
              </button>
            </div>
      ...
      <>
        {[
          ...
          { 
            ...
            statusColor: "bg-emerald-50 text-emerald-600",
            editor: "ธนกฤต แอดมิน",
            editTime: "เมื่อวานนี้, 14:20",
            coverUrl: "https://images.unsplash.com/photo-1612815154858-60aa4c59eaa6?q=80&w=200&auto=format&fit=crop"
          },
        ].map((item, idx) => (
          <div 
            key={idx} 
            className="flex flex-col sm:flex-row items-center justify-between p-5 rounded-[2rem] bg-white/60 border border-slate-100 shadow-sm opacity-90 hover:opacity-100 hover:bg-white transition-all duration-300 group animate-fade-in"
          >
            <div className="flex items-center gap-5 w-full sm:w-auto">
            ...
            <div className="flex items-center gap-4 w-full sm:w-auto justify-between sm:justify-end mt-4 sm:mt-0">
              <span className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest ${item.statusColor} bg-opacity-50 backdrop-blur-sm shadow-sm`}>{item.status}
              </span>
                <button 
                     onClick={() => handleSearch(null, item.id)} 
                    className="px-6 py-2.5 bg-[#6366F1] text-white font-black text-xs rounded-xl hover:bg-[#4F46E5] hover:shadow-lg hover:shadow-indigo-100 active:scale-95 transition-all duration-200 flex items-center gap-2"
                    >
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                                    {currentCase.allImages.filter(img => img.isCover).map((img) => (
                                        <ImageCard 
                                            ...
                                            onSetCover={() => handleSetAsCover(img.id)} 
                                        />
                                    ))}
                                </div>
                            </div>
                    ...
                    <div className="flex items-center justify-center shrink-0">
                        {isSelected ? (
                            <div className="bg-white rounded-full p-0.5 shadow-2xl scale-110">
                                <CheckCircle2 size={30} className="text-indigo-600 fill-white" />
                            </div>
                        ) : (
